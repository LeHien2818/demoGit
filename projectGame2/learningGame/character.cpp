...
void gameCharacter::handdleInputAction(SDL_Event e){
    ...
    if(e.type == SDL_KEYDOWN){
        ...
        p_amo->getPos().x = this->pos.x + 32;
        p_amo->getPos().y = this->pos.y + 32;
 ...
 void gameCharacter::handdleMove(){
    ...
    pos.y += y_val;
    if(pos.y < 0 || pos.y  > WINDOW_HEIGHT - 100 ){
        pos.y -= y_val;
    }
 }
 void gameCharacter::checkMap(GameMap& inMap)
 {
    ...
    x2 = (pos.x + x_val + TILE_SIZE - 1)/TILE_SIZE;

    y1= (pos.y )/TILE_SIZE;
    y2 = (pos.y + TILE_SIZE)/TILE_SIZE;
    ...
    x1 = (pos.x )/TILE_SIZE;
    x2 = (pos.x + TILE_SIZE)/TILE_SIZE;

    y1= (pos.y + y_val)/TILE_SIZE;
    y2 = (pos.y + y_val + TILE_SIZE - 1)/TILE_SIZE;
    if(x1 >= 0 && x2 < MAP_MAX_X && y1 >= 0 && y2 < MAP_MAX_Y )
    {
